...
func buildAuditLogWhereClauses(action, search string, from, to time.Time) (string, []interface{}, int) {
	args := []interface{}{}
	where := []string{}
	idx := 1
	...
	return whereSQL, args, idx
}

func formatAuditLogDetails(detailsRaw string) string {
...
func (r *AnalyticsRepositoryImpl) GetAuditLogPaginated(
	...
	for rows.Next() {
		...
		row.Detail = formatAuditLogDetails(detailsRaw)
