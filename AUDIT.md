...
| 2 | Safe auto-rollout controls | базовый persisted policy control layer и два guarded operator actions уже surfaced, но explicit rollout actions ещё не добиты |
...
| File/path | Checklist |
|---|---|
| `frontend/src/app/(main)/dashboard/experiments/bandit/bandit-page-client.tsx` | ✅ guarded `Confirm winner` и `Hold for review` operator actions surfaced next to recommendation guidance; дальше не превращать страницу в silent auto-rollout surface |
| `frontend/src/app/(main)/dashboard/experiments/studio/studio-page-client.tsx` | ✅ persisted policy editor и guarded recommendation actions (`Confirm winner`, `Hold for review`) рядом с lifecycle/lock controls уже есть; дальше держать explicit/manual semantics |
| `backend/internal/interfaces/http/handlers/admin_experiments.go` + related service layer | ✅ live update path для persisted `automation_policy`, `confirm-winner` и `hold-for-review` endpoints уже есть с admin/lifecycle audit log и respect for `manual_override` / `locked_until`; дальше вводить rollout/action endpoints только при явных guards |
...
| Immutable event / conversion log | теперь есть append-only `experiment_automation_decision_log`, `bandit_conversion_events`, `bandit_assignment_events`, `bandit_impression_events` и `experiment_winner_recommendation_log`; direct reward / delayed conversion / expired pending reward, новые arm assignments, explicit impression calls и admin-facing winner recommendation evaluations уже пишутся в immutable history, а assignment events теперь ещё и несут runtime selection rationale (`selection_strategy`, sampled arm scores, stats source) в metadata | следующий backend шаг уже не про event trail, а про guarded action/policy layer поверх уже имеющейся recommendation history |
