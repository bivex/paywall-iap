...
| 3 | Expand immutable event trail | immutable trail теперь уже покрывает decisions, conversions, assignments и impressions; следующий backend шаг — добить richer recommendation events, чтобы автоматика и operator flows опирались не только на агрегаты |
...
| Immutable event / conversion log | теперь есть append-only `experiment_automation_decision_log`, `bandit_conversion_events`, `bandit_assignment_events` и `bandit_impression_events`; direct reward / delayed conversion / expired pending reward, новые arm assignments и explicit impression calls уже пишутся в immutable history, а assignment path стал repository-backed вместо cache-only | дальше расширять event trail до richer recommendation events, чтобы автоматика опиралась не только на агрегаты |
...
| Immutable decisions/conversions log | 🟡 уже есть append-only история automation decisions, reward-resolution событий, arm assignments и impressions; дальше расширять до recommendation trail |
...
| Stage 3 | Immutable conversions / decisions log | P1 | 🟡 Partial | появились append-only `experiment_automation_decision_log`, `bandit_conversion_events`, `bandit_assignment_events` и `bandit_impression_events`; direct reward / delayed conversion / expired pending reward, новые arm assignments и explicit impressions теперь пишутся в immutable history, но richer recommendation events пока нет |
