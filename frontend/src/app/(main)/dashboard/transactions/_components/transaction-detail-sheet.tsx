...
import { Check, Copy, CreditCard, ExternalLink, Loader2, User } from "lucide-react";
...
export function TransactionDetailSheet({ transactionId, trigger, initialOpen }: Props) {
  ...
  // biome-ignore lint/correctness/useExhaustiveDependencies: fetch on initialOpen mount only
  React.useEffect(() => {
          ...
          {!isPending && detail && (
            <div className="space-y-5 pb-6">
              ...
              <div className="rounded-lg border bg-muted/30 p-4 flex items-center justify-between">
                ...
                <Badge
                  variant="outline"
                  className={`${txStatusClass[detail.status] ?? "bg-muted"} capitalize font-semibold px-3 py-1 text-xs rounded-full`}
                >
                  {detail.status}
                </Badge>
              </div>
              ...
              <SectionCard icon={User} title="User">
                <div className="space-y-3">
                  <div className="grid grid-cols-2 gap-3">
                    <Field
                      label="Email"
                      value={detail.user.email || <span className="text-muted-foreground">—</span>}
                    />
                    <Field
                      label="LTV"
                      value={<span className="text-base font-bold tabular-nums">${detail.user.ltv.toFixed(2)}</span>}
                    />
                  </div>
              ...
              <SectionCard icon={CreditCard} title="Subscription">
                <div className="space-y-3">
                  <div className="flex flex-wrap items-center gap-2">
                    <Badge
                      variant="outline"
                      className={`${subStatusClass[detail.subscription.status] ?? "bg-muted"} capitalize font-semibold px-3 py-0.5 text-xs rounded-full`}
                    >
