...
function ActionBadge({ action }: { action: string }) {
  ...
  return <Badge className={meta?.className ?? "bg-gray-100 text-gray-700"}>{meta?.label ?? action}</Badge>;
}
    ...
    <div className="flex flex-col gap-6">
      <div className="flex items-center justify-between flex-wrap gap-2">
        ...
        <Button variant="outline" size="sm" asChild>
          <Link
            href={`/api/proxy/audit-log/export?action=${sp.action ?? ""}&search=${sp.search ?? ""}&from=${sp.from ?? ""}&to=${sp.to ?? ""}`}
          >
            {t("exportCsv")}
          </Link>
        </Button>
              ...
              {data.rows.length === 0 ? (
                ...
                data.rows.map((row) => (
                  <TableRow key={row.ID}>
                    ...
                    <TableCell className="font-mono text-xs text-muted-foreground">{row.TargetType}</TableCell>
