...
import type { SubscriptionRow as SubscriptionRowData } from "@/actions/subscriptions";
...
export function SubscriptionRow({ s, showCreatedAt }: { s: SubscriptionRowData; showCreatedAt?: boolean }) {
  return (
    <SubscriptionDetailSheet
      ...
      trigger={
        <TableRow className="cursor-pointer hover:bg-muted/50">
          ...
          <TableCell>
            <Badge className={statusClassMap[s.status] ?? "bg-muted text-muted-foreground"}>{s.status}</Badge>
          </TableCell>
