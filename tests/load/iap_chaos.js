 ...
 * IAP Chaos Load Test
 *
 * Simulates 200 concurrent users hitting the IAP verification pipeline
 * through the google-billing-mock server with all 7 scenarios (active,
 * expired, canceled, pending, invalid, product_valid, product_pending).
 *
 * Usage:
 *   k6 run tests/load/iap_chaos.js
 *   k6 run tests/load/iap_chaos.js --env BASE_URL=http://localhost:8081
 *   k6 run tests/load/iap_chaos.js --env VUS=50 --env DURATION=2m
 *
 * Expected outcome per scenario:
 *   valid_active     → 200 OK  (subscription created)
 *   product_valid    → 200 OK  (one-time purchase)
 *   expired          → 422     (receipt is invalid)
 *   canceled         → 422     (receipt is invalid)
 *   pending          → 422     (receipt is invalid)
 *   invalid          → 422     (token no longer valid / 410)
 *   product_pending  → 422     (receipt is invalid)
 */
...
// ──────────────────────────────────────────────
// Chaos scenarios — mirrors mock server prefixes
// ──────────────────────────────────────────────
const SCENARIOS = [
  // [prefix, expectedStatus, label, weight]
...
function register() {
  ...
  const payload = JSON.stringify({
    ...
    platform:         'android',
    app_version:      '1.0.0',
    // no email — tests the empty-email fix
  });
...
function verifyIAP(token, scenario) {
  ...
  const isExpectedStatus = res.status === scenario.expect;
  let parsed = null;
  try { parsed = JSON.parse(res.body); } catch {}

  if (scenario.expect === 200) {
    // Valid receipts: must get active/success subscription back
    const valid = check(res, {
      [`[${scenario.label}] status 200`]:      r => r.status === 200,
      [`[${scenario.label}] has subscription`]:r => parsed && parsed.data && parsed.data.subscription_id,
    });
    ...
    // Invalid receipts: must be rejected with 4xx
    const rejected = check(res, {
      [`[${scenario.label}] rejected (4xx)`]: r => r.status >= 400 && r.status < 500,
      [`[${scenario.label}] has error field`]:r => parsed && parsed.error,
    });
  ...
  return { status: res.status, ok: isExpectedStatus, scenario: scenario.label };
}
...
// ──────────────────────────────────────────────
export default function () {
  // Each VU registers once per iteration (unique user per purchase attempt)
  let token;
  group('register', () => {
    token = register();
  });
  ...
  // Small think time to avoid thundering herd
...
Thresholds:
  iap_valid_success_rate : ${m['iap_valid_success_rate'] ? (m['iap_valid_success_rate'].values.rate * 100).toFixed(1) + '%' : 'n/a'}
  iap_invalid_reject_rate: ${m['iap_invalid_reject_rate'] ? (m['iap_invalid_reject_rate'].values.rate * 100).toFixed(1) + '%' : 'n/a'}
`;
